"""v2 of the panel-mask generator: tells the model to extend white panel fills
all the way to the page edge where panels bleed, rather than drawing a black
frame around the page.

Usage:  python3 gen_panel_mask_v2.py <page.png>  [output_mask.png]
"""
...
ONLY the gutters BETWEEN adjacent panels with PURE BLACK (#000000).
...
- The white/black boundary must trace each panel's actual gutter edges,
  ...
  panel frame.
- **Do NOT draw a black frame, border, or margin around the outside of the
  page.** If a panel touches or bleeds to the page edge (top, bottom, left, or
  right), its white fill must extend all the way to that page boundary — the
  very edge pixel of the output should be white there. Black should appear
  ONLY between two adjacent panels, never as a frame around the page.
...
if __name__ == "__main__":
    ...
    out = pathlib.Path(sys.argv[2]) if len(sys.argv) > 2 else src.with_name(f"{src.stem}_mask_v2.png")
    generate(src, out)
